...
namespace Core.Models
{
    ...
    {
        private static string shorturl_chars_lcase = "abcdefghijkmlnopqrstwxyz-";
        private static string shorturl_chars_ucase = "ABCDEFGHIJKLMNOPQRSTWXYZ-";
        private static string shorturl_chars_numeric = "23456789";

        public static string GetShortUrl(this Article article)
        {
            string short_url = RandomCharacters();
            var count = new DataProvider().GetArticleShortUrls(short_url);

            if (count == 0)
                return short_url;

            while (count > 0)
                return GetShortUrl(article);
        ...
        private static string RandomCharacters()
        {
            ...
            short_url = new char[random.Next(7, 7)];
